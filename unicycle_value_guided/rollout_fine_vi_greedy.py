...
def _plot_episode(
    *,
    ...
    ax.scatter([float(goal_xy[0])], [float(goal_xy[1])], c="r", s=60, marker="*", label="goal")

    if states.shape[0] >= 2:
        ax.plot(states[:, 0], states[:, 1], c="w", linewidth=2.0, alpha=0.95)
    ax.scatter([float(states[0, 0])], [float(states[0, 1])], c="w", s=30, marker="o", label="start")

    # Draw footprint for last state.
    xL, yL, yawL = float(states[-1, 0]), float(states[-1, 1]), float(states[-1, 2])
    corners = footprint_corners_xy(x=xL, y=yL, yaw=yawL, length_m=footprint_length_m, width_m=footprint_width_m)
    ax.add_patch(Polygon(corners, closed=True, edgecolor="w", facecolor="none", linewidth=1.0, alpha=0.9))
...
def _build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Re-run a VI-greedy rollout using fine VI graph (level6), reusing the same start states from an existing "
            "infer-like output directory (episode_*_states.npy). If a collision happens, stop the episode immediately "
            "and move on to the next start."
        )
    )
    p.add_argument("--infer-dir", type=str, required=True, help="Path to an existing output dir that contains episode_*_states.npy.")
    p.add_argument("--out-dir", type=str, default=None, help="Output directory. Default: <goal_dir>/fine_vi_greedy_swept_<tag>.")
    ...
    p.add_argument("--nearby-radius", type=float, default=0.25, help="Radius for interpolation=nearby (meters).")
    p.add_argument(
        "--unreachable-eps",
        type=float,
        default=1e-4,
    ...
    p.add_argument("--plot", action="store_true", help="Also write episode_*.png visualizations (background uses coarse value grid).")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not meta_fine_path.exists():
        raise FileNotFoundError(f"Missing meta_fine.json next to infer dir: {meta_fine_path}")
    meta_fine = json.loads(meta_fine_path.read_text(encoding="utf-8"))

    task = load_task(meta_fine.get("task_path", meta_fine.get("tmp_task_path")))
    ...
    if bool(getattr(args, "opt_a", False)):
        ...
        if not meta_coarse_path.exists():
            raise FileNotFoundError(f"Opt-A requested but meta_coarse.json is missing next to infer dir: {meta_coarse_path}")
        meta_coarse = json.loads(meta_coarse_path.read_text(encoding="utf-8"))

        map_name = str(src_summary.get("map_name", meta_fine.get("map_name", goal_dir.parent.name)))
        goal_index = int(src_summary.get("goal_index", meta_fine.get("goal_index", 0)))
        goal_pose = src_summary.get("goal_pose", meta_fine.get("goal_pose", [0.0, 0.0, 0.0]))
        base_task_path = meta_fine.get("task_path", None) or meta_fine.get("tmp_task_path", None)
        if base_task_path is None:
            raise RuntimeError("Opt-A requested but meta_fine has no task_path/tmp_task_path.")
    ...
    # VI robot (fine) provides KDTree, node values and goal semantics.
    ...
    for sp in state_paths:
        ...
        if arr.ndim != 2 or arr.shape[1] != 3 or arr.shape[0] < 1:
            raise ValueError(f"Invalid states file: {sp} shape={arr.shape}")
        starts_yaw.append(arr[0].astype(np.float32, copy=True))

    # Output directory.
    out_dir = None if args.out_dir is None else Path(str(args.out_dir)).expanduser()
    if out_dir is None:
        tag = f"{str(args.collision).strip().lower()}_{infer_dir.name}"
        out_dir = goal_dir / f"fine_vi_greedy_{tag}"
    ...
    goal_pose = np.asarray(src_summary.get("goal_pose", meta_fine.get("goal_pose", [0.0, 0.0, 0.0])), dtype=np.float64).reshape(3)
    goal_xy = goal_pose[:2]
    ...
    def _node_optimal_control(idx: int) -> np.ndarray:
        ...
        node = fine_robot.nodes[idx]
        children = list(getattr(node.children, "indices", []))
        if not children:
            ctrl_cache[idx] = np.zeros(2, dtype=np.float32)
            return ctrl_cache[idx]
        child_vals = [float(getattr(fine_robot.nodes[int(c)], "value", 1.0)) for c in children]
        ...
        for c in candidates:
            nxt_state = np.asarray(fine_robot.nodes[int(c)].state, dtype=np.float32).reshape(3)
    ...
    def _get_control(state_scaled: np.ndarray) -> tuple[np.ndarray | None, list[int]]:
        ...
        if str(args.interpolation) == "nearest":
            ref_node_indices = list(getattr(fine_robot, "query_kdtree")(state_scaled))
            ref_node_indices = ref_node_indices[:1]
        else:
            ref_node_indices = list(getattr(fine_robot, "query_kdtree")(state_scaled, radius=float(args.nearby_radius)))

        ref_nodes: list[int] = []
        for i in ref_node_indices:
            if abs(float(getattr(fine_robot.nodes[int(i)], "value", 1.0)) - 1.0) <= float(args.unreachable_eps):
                continue
    ...
    for ep, start_yaw in enumerate(starts_yaw):
        ...
        if bool(getattr(fine_robot, "within_goal")(s)):
            reason = "reached_goal"
            success = True
        else:
            for _ in range(int(args.max_steps)):
                u, _refs = _get_control(s)
                if u is None:
                    reason = "unreachable"
                    break
                ok_collision = False
        ...
        if bool(args.plot):
            ...
            _plot_episode(
                ...
                title=f"fine_vi_greedy | {reason} | T={T}",
            )

        per_episode.append({"episode": ep, "success": bool(success), "reason": reason, "T": T})
        print(f"[fine_vi_greedy] ep={ep} success={success} reason={reason} T={T}", flush=True)

    out_summary = {
        "map_name": str(src_summary.get("map_name", meta_fine.get("map_name", ""))),
        "goal_index": int(src_summary.get("goal_index", meta_fine.get("goal_index", 0))),
    ...
    print(f"[fine_vi_greedy] wrote summary: {out_dir / 'summary.json'}", flush=True)
