...
def load_task(task_path: str | Path) -> dict[str, Any]:
    ...
    if "env" not in task or "robots" not in task:
        raise ValueError(f"Invalid task json (missing env/robots): {task_path}")
    return task
...
def get_range_limits(task: dict[str, Any]) -> tuple[tuple[float, float], tuple[float, float]]:
